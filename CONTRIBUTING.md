...
[httpie](https://httpie.io/docs#main-features), a user friendly client. Note: The API key used to authenticate this request is hardcoded into the example here because the example address whose ID we're using can only be accessed with this particular key (which is from the docs).

Anatomy of the HTTPie request:

- `-a <API key>`: used for authentication
...
- `adr_fa85158b26c3eb7c`: the ID of the address we want to retrieve (taken from the docs example)
...
```bash
$ http -a test_0dc8d51e0acffcb1880e0f19c79b2f5b0cc: -v GET http://127.0.0.1:4010/addresses/adr_fa85158b26c3eb7c Accept-Encoding:
...
An important note: `npm run proxy` no longer includes the `--errors` flag, because the errors generated by [Prism](https://meta.stoplight.io/docs/prism/docs/guides/errors.md)
do not match the required format of our errors. If you run Prism's proxy server with `--errors` and invalid input,
you will receive a cryptic 500 error.
