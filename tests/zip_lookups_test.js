...
test('lookup a US zip code', async function(t) {
  const response = await prism.setup()
    .then(client => client.post(resource_endpoint, { zip_code: zip }, { headers: prism.authHeader }));
...
test('use an incorrectly formatted zip code', async function(t) {
  const response = await prism.setup()
    .then(client => client.post(resource_endpoint, { zip_code: '123456' }, { headers: prism.authHeader }));
